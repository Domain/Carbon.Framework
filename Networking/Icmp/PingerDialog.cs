...
namespace Carbon.Networking.Icmp
{
	...
	public class PingerDialog : System.Windows.Forms.Form
	{
		...
		public PingerDialog(string address, bool autoStartPinging)
		{
			...
			_pinger = new Pinger();
			_pinger.Exception += new EventHandler<ExceptionEventArgs>(OnPingException);
			_pinger.PingStarted += new EventHandler<PingerEventArgs>(OnPingStarted);
			_pinger.PingResult += new EventHandler<PingerResultEventArgs>(OnPingResult);
			_pinger.PingStatistics += new EventHandler<PingerStatisticsEventArgs>(OnPingStatistics);
